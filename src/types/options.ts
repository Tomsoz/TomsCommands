...
> = RequiredOption<T> | OptionalOption<T>;
...
export type TransformOptions<O extends Options> = {
	[K in keyof O]: O[K] &
		(O[K] extends { type: infer T extends keyof PrimitiveTypeMap }
			? O[K] extends { choices: ReadonlyArray<{ value: infer V }> }
				? O[K] extends { required: true }
					? { value: O[K]["choices"][number]["value"] }
					: O[K] extends { default: any }
					? { value: O[K]["choices"][number]["value"] }
					: { value?: O[K]["choices"][number]["value"] }
				: O[K] extends { required: true }
				? { value: PrimitiveTypeMap[T] }
